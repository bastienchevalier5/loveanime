<?php
session_start();
include("bd.php");
?>
<!doctype html>
<html lang="en">
    <head>
        <title>Tokyo Revengers</title>
        <!-- Required meta tags -->
        <meta charset="utf-8" />
        <meta
            name="viewport"
            content="width=device-width, initial-scale=1, shrink-to-fit=no"
        />

        <!-- Bootstrap CSS v5.2.1 --> 
        <link
            href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css"
            rel="stylesheet"
            integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN"
            crossorigin="anonymous"
        />
        <link rel="stylesheet" href="CSS/loveanime.css">
    </head>

    <body>
        <header>
            <?php
            include("header.php");
            ?>
        </header>
        <main>
            <div class="card" style="background-color:#ebebeb">
                <div class="row g-0 w-100">
                  <div class="col m-3">
                    <img src="images/tr.jpg" class="img-fluid rounded-start h-100 ms-5" alt="tr" title="tr">
                  </div>
                  <div class="col-md-8">
                        <div class="card-body text-start border m-5" style="background-color: #F6F6F6;">
                            <h5 class="card-title">Tokyo Revengers</h5>
                            <div class="card-text">Titre original : 東京リベンジャーズ
                                      </br>Saison 1 : 24 épisodes
                                      </br>Saison 2 : 13 épisodes
                                      </br>Saison 3 : 10 épisodes (en cours)
                                      </br>Statut : En cours
                                      </br>Genres : Action - Drame - Romance - School Life
                                      </br>Thèmes : Amour - Délinquance - Ecole - Mort - Vengeance - Voyage temporel
                                      </br>Studio d'animation : Liden Films
                                      </br>Disponible sur : <a href="https://www.crunchyroll.com/fr/series/G3KHEVMN1/tokyo-revengers"><img src=images/crunchyroll.png alt="crunchyroll" title="crunchyroll"></a></div>
                        </div>
                        <div class="card text-bg-light m-5">
                            <div class="card-header text-white" style="background-color: #4F76BB;">Synopsis</div>
                            <div class="card-body">
                              <p class="card-text">Takemichi Hanagaki n'a pas vraiment réussi sa vie. Il fait des petits boulots sans intérêt et lorsqu'il apprend que la seule petite amie qu'il ait jamais eue au collège, Hinata Tachibana, a été tuée par l'impitoyable gang Tokyo Manji-kai, il est anéanti. Le lendemain, toujours sous le choc, il est poussé sur les rails de la gare lors d'un mouvement de foule. Mais Takemichi ne meurt pas et quand il se réveille, il est revenu douze ans en arrière.</p>
                            </div>
                          </div>
                        </div>
                        <div class="card text-bg-light mb-3">
                            <div class="card-header text-white" style="background-color: #4F76BB;">Personnages</div>
                            <div class="card-body text-center">
                              <p class="card-text">
                                <div class="container text-center">
                                    <div class="row align-items-center">
                                        <div class="col">
                                            <img style="height: 150px;" src="images/takemichi.png" alt="takemichi" title="takemichi">
                                            <p>Hanagaki Takemichi</p>
                                        </div>
                                        <div class="col">
                                            <img style="height: 150px;" src="images/mikey.jpg" alt="mikey" title="mikey">
                                            <p>Sano Manjirô (Mikey)</p>
                                        </div>
                                        <div class="col">
                                            <img style="height: 150px;" src="images/draken.jpg" alt="draken" title="draken">
                                            <p>Ken Ryûgûji (Draken)</p>
                                        </div>
                                        <div class="col">
                                            <img style="height: 150px;" src="images/hina.jpg" alt="hina" title="hina">
                                            <p>Tachibana Hinata (Hina)</p>
                                        </div>
                                        <div class="col">
                                            <img style="height: 150px;" src="images/naoto.jpg" alt="naoto" title="naoto">
                                            <p>Tachibana Naoto</p>
                                        </div>
                                        <div class="col">
                                          <img style="height: 150px;" src="images/kisaki.jpg" alt="kisaki" title="kisaki">
                                          <p>Kisaki Tetta</p>
                                        </div>
                                    </div>
                                </div>
                              </p>
                            </div>
                        </div>
                        <div class="card text-bg-light mb-3">
                          <div class="card-header text-white" style="background-color: #4F76BB;">Episodes</div>
                            <div class="card-body" style="max-height: 300px;overflow-y: auto;">
                              <p class="card-text">
                                <table class="table table-bordered caption-top text-center"><h6>Saison 1</h6>
                                  <thead>
                                    <tr>
                                      <th scope="col">N°</th>
                                      <th scope="col">Titre français</th>
                                      <th scope="col">Titre japonais</th>
                                      <th scope="col">Date</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr>
                                      <th scope="row">1</th>
                                      <td>Reborn</td>
                                      <td>Reborn</td>
                                      <td>11/04/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">2</th>
                                      <td>Resist</td>
                                      <td>Resist</td>
                                      <td>18/04/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">3</th>
                                      <td>Resolve</td>
                                      <td>Resolve</td>
                                      <td>25/04/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">4</th>
                                      <td>Return</td>
                                      <td>Return</td>
                                      <td>02/05/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">5</th>
                                      <td>Releap</td>
                                      <td>Releap</td>
                                      <td>09/05/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">6</th>
                                      <td>Regret</td>
                                      <td>Regret</td>
                                      <td>16/05/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">7</th>
                                      <td>Revive</td>
                                      <td>Revivde</td>
                                      <td>23/05/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">8</th>
                                      <td>Rechange</td>
                                      <td>Rechange</td>
                                      <td>30/05/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">9</th>
                                      <td>Revolt</td>
                                      <td>Revolt</td>
                                      <td>06/06/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">10</th>
                                      <td>Rerise</td>
                                      <td>Rerise</td>
                                      <td>13/06/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">11</th>
                                      <td>Respect</td>
                                      <td>Respect</td>
                                      <td>20/06/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">12</th>
                                      <td>Revenge</td>
                                      <td>Revenge</td>
                                      <td>27/06/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">13</th>
                                      <td>Odds and Ends</td>
                                      <td>Odds and Ends</td>
                                      <td>04/07/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">14</th>
                                      <td>Break up</td>
                                      <td>Break up</td>
                                      <td>11/07/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">15</th>
                                      <td>No pain, no gain</td>
                                      <td>No pain, no gain</td>
                                      <td>18/07/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">16</th>
                                      <td>Once upon a time</td>
                                      <td>Once upon a time</td>
                                      <td>25/07/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">17</th>
                                      <td>No way</td>
                                      <td>No way</td>
                                      <td>01/08/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">18</th>
                                      <td>Open Fire</td>
                                      <td>Open Fire</td>
                                      <td>08/08/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">19</th>
                                      <td>Turn around</td>
                                      <td>Turn around</td>
                                      <td>15/08/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">20</th>
                                      <td>Dead or Alive</td>
                                      <td>Dead or Alive</td>
                                      <td>22/08/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">21</th>
                                      <td>One and only</td>
                                      <td>One and only</td>
                                      <td>29/08/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">22</th>
                                      <td>One for all</td>
                                      <td>One for all</td>
                                      <td>05/09/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">23</th>
                                      <td>End of war</td>
                                      <td>End of war</td>
                                      <td>12/09/2021</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">24</th>
                                      <td>A cry baby</td>
                                      <td>A cry baby</td>
                                      <td>19/09/2021</td>
                                    </tr>
                                  </tbody>
                                </table>
                                <table class="table table-bordered caption-top text-center"><h6>Saison 2</h6>
                                  <thead>
                                    <tr>
                                      <th scope="col">N°</th>
                                      <th scope="col">Titre français</th>
                                      <th scope="col">Titre japonais</th>
                                      <th scope="col">Date</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr>
                                      <th scope="row">1</th>
                                      <td>C'est comme ça</td>
                                      <td>It is what it is</td>
                                      <td>08/01/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">2</th>
                                      <td>Faut y aller</td>
                                      <td>Gotta go</td>
                                      <td>15/01/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">3</th>
                                      <td>Autonome</td>
                                      <td>Stand alone</td>
                                      <td>22/01/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">4</th>
                                      <td>Liens familiaux</td>
                                      <td>Family bonds</td>
                                      <td>29/01/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">5</th>
                                      <td>Le réveillon</td>
                                      <td>Christmas eve</td>
                                      <td>05/02/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">6</th>
                                      <td>Remonter le morale des troupes</td>
                                      <td>Whip up morale</td>
                                      <td>12/02/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">7</th>
                                      <td>Rivalité fraternelle</td>
                                      <td>Sibling rivalry</td>
                                      <td>19/02/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">8</th>
                                      <td>Faire front commun</td>
                                      <td>Strive together</td>
                                      <td>26/02/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">9</th>
                                      <td>À l'aube d'une nouvelle ère</td>
                                      <td>Drawning of a new era</td>
                                      <td>05/03/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">10</th>
                                      <td>La lumière de ma vie</td>
                                      <td>The light of my life</td>
                                      <td>12/03/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">11</th>
                                      <td>Je rentre chez moi</td>
                                      <td>On my way home</td>
                                      <td>19/03/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">12</th>
                                      <td>Le dernier ordre</td>
                                      <td>Last order</td>
                                      <td>26/03/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">13</th>
                                      <td>Quand il pleut, il pleut des cordes</td>
                                      <td>When it rains, it pours</td>
                                      <td>02/04/2023</td>
                                    </tr>
                                  </tbody>
                                </table>
                                <table class="table table-bordered caption-top text-center"><h6>Saison 3</h6>
                                  <thead>
                                    <tr>
                                      <th scope="col">N°</th>
                                      <th scope="col">Titre français</th>
                                      <th scope="col">Titre japonais</th>
                                      <th scope="col">Date</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr>
                                      <th scope="row">1</th>
                                      <td>La journée la plus longue</td>
                                      <td>The longest day</td>
                                      <td>04/10/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">2</th>
                                      <td>Ennemi mortel</td>
                                      <td>Mortal enemy</td>
                                      <td>11/10/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">3</th>
                                      <td>La patience a ses limites</td>
                                      <td>Run out of patience</td>
                                      <td>18/10/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">4</th>
                                      <td>Le retour à la vie</td>
                                      <td>Come back to life</td>
                                      <td>25/10/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">5</th>
                                      <td>Un mauvais pressentiment</td>
                                      <td>A bad hunch</td>
                                      <td>01/11/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">6</th>
                                      <td>Debout !</td>
                                      <td>Rise against</td>
                                      <td>08/11/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">7</th>
                                      <td>Changement de courant</td>
                                      <td>Turn the tide</td>
                                      <td>15/11/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">8</th>
                                      <td>Je le sais au fond de moi</td>
                                      <td>I know in my head</td>
                                      <td>22/11/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">9</th>
                                      <td>L'ogre bleu</td>
                                      <td>The blue ogre</td>
                                      <td>29/11/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">10</th>
                                      <td>Le cœur brave</td>
                                      <td>Brave heart</td>
                                      <td>06/12/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">11</th>
                                      <td>Quand il ne reste plus rien</td>
                                      <td>Nothing is left</td>
                                      <td>13/12/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">12</th>
                                      <td>Paradis perdu</td>
                                      <td>Paradise lost</td>
                                      <td>20/12/2023</td>
                                    </tr>
                                    <tr>
                                      <th scope="row">13</th>
                                      <td>Son destin</td>
                                      <td>Meet his fate</td>
                                      <td>27/12/2023</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>

        </main>
        <footer>
            <?php
            include("footer.php");
            ?>
        </footer>
        <!-- Bootstrap JavaScript Libraries -->
        <script
            src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js"
            integrity="sha384-I7E8VVD/ismYTF4hNIPjVp/Zjvgyol6VFvRkX/vR+Vc4jQkC+hVqc2pM8ODewa9r"
            crossorigin="anonymous"
        ></script>

        <script
            src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.min.js"
            integrity="sha384-BBtl+eGJRgqQAUMxJ7pMwbEyER4l1g+O15P+16Ep7Q9Q+zqX6gSbd85u4mG4QzX+"
            crossorigin="anonymous"
        ></script>
    </body>
</html>
